...
export function toCalculatorAttributes(cls: StartingClass): Attributes {
  return {
...
function StartingWeaponAttack({ cls }: { cls: StartingClass }) {
  ...
  const result = getWeaponAttack({
    weapon,
    attributes: toCalculatorAttributes(cls),
    upgradeLevel: 0,
  });
...
function ClassDetail({
  ...
  cls: StartingClass;
  onUseStats: (cls: StartingClass) => void;
}) {
  ...
  return (
    <div className="detail">
      <div className="detail-head">
        <h2>{cls.name}</h2>
        <button className="ghost-button" onClick={() => onUseStats(cls)}>
          Browse with these stats →
        </button>
      </div>
...
export function ClassesView({ onUseStats }: { onUseStats: (cls: StartingClass) => void }) {
  ...
  return (
    <div className="layout">
      ...
      {selected && <ClassDetail cls={selected} onUseStats={onUseStats} />}
    </div>
  );
}
